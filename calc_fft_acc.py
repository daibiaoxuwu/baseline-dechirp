...
acc = 0
print('=============SNR:'+str(opts.snr)+'=================')
print('| {:^10} | {:^10} | {:^10}'.format('COUNT', 'CORRECT', 'ACCURACY'))
for cnt in range(1, opts.n_classes * 100 + 1):
    truth_idx = cnt % opts.n_classes # the real symbol of chirp
...
# calculate 
    estimate_idx = np.argmax(fft_data)
    if(estimate_idx == truth_idx): acc+=1
    if cnt % opts.n_classes == 0: print('| {:^10} | {:^10} | {:^10.3f}'.format(cnt, acc, acc/cnt))
